#!/usr/bin/env python3
"""
测试逐步生成模式下的Mask预测准确率（使用FlexAttention）

模式：
1. 已生成的token: [Prompt][R1][R2]...
2. 拼接3个Mask: [Prompt][R1][R2]...[M][M][M]
3. 使用FlexAttention，传递block_info = [('real', len(已生成)), ('mask', 3)]
4. 预测这3个Mask位置 -> 得到预测的[R_next]
5. Teacher forcing: 用正确的token替换 -> [Prompt][R1][R2]...[R_correct]
6. 继续下一个block

这模拟了真实的生成场景，但使用了和训练一致的FlexAttention机制。
"""
...
def test_sequential_generation_with_flex(
    model,
    tokenizer,
    ...
    print(f"{'='*80}")
    print(f"逐步生成模式测试（使用FlexAttention + Teacher Forcing）")
    print(f"{'='*80}")
    ...
    for block_idx in range(num_blocks):
        ...
        # Step 3: 构造block_info
        # 格式: [('real', len(已生成)), ('mask', 3)]
        # ⚠️ 这里很关键：前面的所有token（包括prompt）都标记为'real'
        # 这样FlexAttention会创建合适的mask：
        # - Mask可以看到所有前面的real tokens（包括prompt）
        # - Mask之间是causal的
        current_len = current_generated.size(0)
        block_info_for_model = [
            ('real', current_len),   # 前面的所有tokens（prompt + 已生成）
            ('mask', num_masks),      # 3个mask tokens
        ]
        ...
        # Step 5: 前向传播（使用FlexAttention）
        # ⚠️ 和训练一样，传递block_info让模型内部创建BlockMask
        with torch.no_grad():
            outputs = model(
                input_ids,
                position_ids=position_ids_input,
                # 传递参数让模型内部创建BlockMask
                block_info=[block_info_for_model],
                prompt_len=[current_len],  # 这里prompt_len是已生成长度
        ...
        # Step 7: Teacher forcing - 用正确的token替换
...
def main():
    ...
    print(f"✅ 模型加载完成（train mode - 使用FlexAttention）\n")

    # 运行测试
    test_sequential_generation_with_flex(
        model=model,
        tokenizer=tokenizer,
