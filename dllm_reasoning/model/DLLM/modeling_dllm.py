...
class DLLMForCausalLM(DLLMPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        # ========== 检测是否使用 Interleaved Training 模式 ==========
        # Interleaved Training 通过 FlexAttention BlockMask 提供预处理好的数据
        # Block Diffusion 在模型内部动态插入 mask tokens
        # ⚠️ 已弃用：我们现在在 Dataset 层面预处理数据，不再需要模型内部的 Block Diffusion 逻辑
        # try:
        ...
        # ========== Block Diffusion 训练路径（已注释掉）==========
        # 原有逻辑：在模型内部动态插入 mask tokens
        # 现在我们在 Dataset 层面预处理数据成 [P][M][R][M][R]... 格式，更解耦、更灵活
        ...
        if self.training:
            # ========== Interleaved Training 训练路径（新逻辑）==========
            # 数据已经在 Dataset 层面预处理成 [P][M][R][M][R]... 格式
            # attention_mask 是 FlexAttention BlockMask，定义了自定义注意力规则
            # 不需要 prepare_for_bd_training()，直接前向传播
            assert inputs_embeds is None, "only support input_ids during training"

            outputs = self.model(
                input_ids=input_ids,
                attention_mask=attention_mask,  # FlexAttention BlockMask
                position_ids=position_ids,
                output_attentions=output_attentions,
                output_hidden_states=output_hidden_states,
                return_dict=True,
            ...
            # ✅ 方案A：完全解耦 - 不在模型内部计算 loss
            # 将 hidden_states 返回给 Trainer，由 Trainer 负责：
            # 1. 计算 logits = lm_head(hidden_states)
            # 2. 计算 loss = CrossEntropyLoss(logits, labels)
            # 这样可以在测试中验证 logits 和标签对齐

            # 注意：为了兼容 HuggingFace Trainer API，我们返回 logits 而不是 hidden_states
            # 但实际上这里的 "logits" 是 hidden_states（未经过 lm_head）
            # Trainer 会负责应用 lm_head
